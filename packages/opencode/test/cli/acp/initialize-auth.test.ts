...
describe("codius acp initialize/auth subprocess", () => {
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("Codius")
        expect(initialized.agentInfo?.version).toBeTruthy()
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.authMethods?.[0]?.id).toBe("codius-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toEqual({
          command: "codius",
          args: ["providers", "login", "--provider", "codius"],
          label: "Codius Login",
        })
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "codius-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(
          process.env.CODIUS_API_KEY ?? process.env.OPENCODE_AUTH_CONTENT ?? "not-present",
        )
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("codius-login")
